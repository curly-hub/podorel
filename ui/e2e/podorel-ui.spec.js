...
test.describe.serial('PoDorel full UI click coverage', () => {
  ...
  test('templates catalog and create-pod builder controls', async ({ page }) => {
    ...
    const saveTemplateResponse = page.waitForResponse((response) => response.request().method() === 'POST' && response.url().endsWith('/api/templates') && response.status() === 200);
    await page.getByRole('button', { name: /Save template/i }).click();
    await saveTemplateResponse;
    await expect(page.getByText('Saved E2E Template.')).toBeVisible();
    const savedTemplateCard = page.locator('.catalog-card').filter({ hasText: 'e2e-template' });
    await expect(savedTemplateCard.getByText('Saved in PoDorel')).toBeVisible();
    await savedTemplateCard.getByRole('button', { name: 'Use' }).click();
    await expect(page).toHaveURL(/\/create-pod\?template=e2e-template/);
    await page.goBack();
    await expect(page).toHaveURL(/\/templates$/);
    await savedTemplateCard.getByRole('button', { name: /Delete/i }).click();
    await expect(savedTemplateCard).toBeHidden();
